        ...
        <div class="flex items-center justify-between gap-10">
            ...
            <div class="w-screen h-screen -mx-10">
                <div class="drop-shadow-lg rounded px-10 py-8 bg-white h-full w-full">
                    ...
                    <form action="{{ route('auth.register_mitra.store') }}" method="POST" x-data="{ nama: '', bidang_usaha: false, email: '', password: '' }">
                        @csrf

                        <div class="flex justify-between gap-5 flex-col lg:flex-row">
                            <div class="block mb-2 lg:mb-5 w-full">
                                <label for="nama" class="block mb-1 font-medium">Nama Perusahaan<code
                        ...
                        <div class="flex justify-between gap-5 flex-col lg:flex-row">
                            <div class="block mb-2 lg:mb-5 w-full">
                                <label for="bidang_usaha" class="block mb-1 font-medium">Bidang Usaha<code
                                        class="text-red-600">*</code></label>
                                <select data-te-select-init id='bidang_usaha' name='bidang_usaha_id' x-model="bidang_usaha"
                                    required>
                                    <option value="" hidden selected></option>
                                    @foreach ($bidang_usaha as $data)
                                        <option value="{{ $data->id }}">{{ $data->title }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('bidang_usaha_id'))
                                    <p class="text-red-500 mb-3 text-sm">{{ $errors->first('bidang_usaha_id') }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="block mt-5 mb-3">
                            <button type="submit" class="h-11 w-full rounded-md bg-blue-700 text-white block"
                                id="fill"
                                x-show="nama.length && bidang_usaha && email.length && password.length">Daftar</button>

                            <div class=" w-full rounded-md bg-gray-500 cursor-not-allowed text-white text-center py-2.5"
                                x-show="!nama.length || !bidang_usaha || !email.length || !password.length"
