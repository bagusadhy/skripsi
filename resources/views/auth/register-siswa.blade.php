        ...
        <div class="flex items-center justify-between gap-10">
            ...
            <div class="w-screen h-screen -mx-10">
                <div class="drop-shadow-lg rounded px-10 py-8 bg-white h-full w-full">
                    ...
                    <form action="{{ route('auth.register_siswa.store') }}" method="POST" x-data="{ nama: '', nisn: '', jurusan: false, kelas: false, email: '', password: '' }">
                        @csrf
                        ...
                        <div class="flex justify-between gap-5 flex-col lg:flex-row">
                            ...
                            <div class="block mb-2 lg:mb-5 w-full">
                                <label for="nisn" class="block mb-1 font-medium">NISN<code
                                        class="text-red-600">*</code></label>
                                <input type="number" name="nisn" id="nisn" value="{{ old('nisn') }}"
                                    ...
                                    x-model="nisn" required autofocus>
                                @if ($errors->has('nisn'))
                                    <p class="text-red-500 mb-3 text-sm">{{ $errors->first('nisn') }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between gap-5 flex-col lg:flex-row">
                            ...
                            <div class="block mb-2 lg:mb-5 w-full">
                                <label for="kelas" class="block mb-1 font-medium">Kelas<code
                                        class="text-red-600">*</code></label>
                        ...
                        <div class="block mt-5 mb-3">
                            <button type="submit" class="h-11 w-full rounded-md bg-blue-700 text-white block"
                                id="fill"
                                x-show="nama.length && nisn.length && jurusan && kelas && email.length && password.length">Daftar</button>

                            <div class=" w-full rounded-md bg-gray-500 cursor-not-allowed text-white text-center py-2.5"
                                x-show="!nama.length || !nisn.length || !jurusan || !kelas || !email.length || !password.length"
