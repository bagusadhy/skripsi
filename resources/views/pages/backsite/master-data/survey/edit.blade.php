        ...
        <div class="sm:px-4">
            <div class="px-4 py-4 md:py-7">
                <div tabindex="0"
                    ...
                    <a href="{{ route('backsite.survey.index') }}" class="flex items-center">
            ...
            <div class="px-5 py-4 mb-10">
                <form action="{{ route('backsite.survey.update', $survey->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <textarea class="w-full h-20 rounded-md border border-gray-300 px-5 py-3 focus:outline-none resize-none"
                        name="title"id="title" required>{{ $survey->title }}</textarea>
                    @if ($errors->has('title'))
