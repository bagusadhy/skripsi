...
@section('content')
    <main class="basis-10/12 bg-white min-h-fit px-10 py-5  overflow-x-hidden">
        ...
        <div class="mb-10">
            ...
            <div class="mt-5 mb-10 w-full px-5 py-5 rounded bg-gray-200">
                <h4 class="font-bold">Catatan:</h4>
                <p>{{ $logbook->revisi }}</p>
            </div>

            <form action="{{ route('siswa.kegiatan.update', $logbook->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div>
                    <label class="relative block mb-5">
                        ...
                        <p>{{ date('d M Y', strtotime($logbook->tanggal)) }}</p>
                        </div>
                ...
                <div>
                    <label for="" class="block mb-5">
                        ...
                        <textarea id="" class="w-full h-44 border-2 border-gray-200 rounded-md resize-none p-4 focus:outline-none"
                            name="jurnal" placeholder="Deskripsi Kegiatan" required>{{ $logbook->jurnal }}</textarea>
                    </label>
